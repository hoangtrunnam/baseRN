...
export function moneyUnits(amount: number | undefined, isMap?: boolean): string {
  if (amount) {
    ...
    if (amount >= 1000000) {
      ...
      return `${money.toFixed(fixedNumber > 0 ? 2 : 0)} ${isMap ? 'tr' : 'triệu'}`.replace('.', ',')
    }
    if (amount >= 1000) {
...
/** func format tiền chẵn */
export function moneyUnitsRound(amount: number | undefined, value: number = 1): string {
  if (amount) {
    if (amount >= 1000000000) {
      const money = amount / 1000000000
      return `${money.toFixed(value)} tỷ`
        .replace('.', ',')
        .replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1.')
    }
    if (amount >= 1000000) {
      const money = amount / 1000000
      return `${money.toFixed(value)} triệu`
        .replace('.', ',')
        .replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1.')
    }
    if (amount >= 1000) {
      const money = amount / 1000
      return `${money.toFixed(value)} nghìn`
        .replace('.', ',')
        .replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1.')
    }
    if (amount >= 0) {
...
/** func làm tròn số */
export function unitsRound(amount: number | undefined): string {
  if (amount) {
    if (amount >= 1000000000) {
      const money = amount / 1000000000
      return `${money.toFixed(0)}`.replace('.', ',').replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1.')
    }
    if (amount >= 1000000) {
      const money = amount / 1000000
      return `${money.toFixed(0)}`.replace('.', ',').replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1.')
